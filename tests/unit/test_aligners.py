...
class TestAlign(TestCase):
    def test(self):
        ...
        np.testing.assert_array_almost_equal(
            new_structure.get_coordinates(),
            np.array(
                [
                    [-114.23727413, -8.65418163, 3.78295986],
...
class TestAlignPandas(TestCase):
    def test(self):
        ...
        new_df = align_pandas_structure(
            df_1[['pos_x', 'pos_y', 'pos_z']].to_numpy(), df_2[['pos_x', 'pos_y', 'pos_z']].to_numpy(), df_1
        )

        np.testing.assert_array_almost_equal(
            new_df[['pos_x', 'pos_y', 'pos_z']].to_numpy(),
            np.array(
                [
                    [-114.23727413, -8.65418163, 3.78295986],
                    [-113.03039812, -8.00613587, 3.19168835],
